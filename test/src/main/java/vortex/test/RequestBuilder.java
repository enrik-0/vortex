...
class RequestBuilder{
    ...
    public
    RequestBuilder(Map<String, String> headers){
    ...
    private static
    Response createResponse(HttpURLConnection connection, InputStream input) throws IOException{

        Response response = new ResponseStatus<Object>(getResponseBody(connection, input));
    ...
    private static
    Object getResponseBody(HttpURLConnection connection, InputStream inputStream) throws IOException{
        try{
            Object mapped = null;
            if(inputStream != null){
                ...
                if("application/json".equals(contentHeader)){
                    ...
                    try{
                        mapped = MappingUtils.map(byteArrayOutputStream.toByteArray(), Map.class);
                    }catch(Exception e){
                        mapped = MappingUtils.map(byteArrayOutputStream.toByteArray(), ArrayList.class);
                    }
                }
                else{
    ...
    private static
    Object getResponseBody(byte[] content) throws IOException{
        Long temp;
        char[] chars;
        Object body = null;
        var builder = new StringBuilder();
        String buffer = MappingUtils.getInputContent(new ByteArrayInputStream(content));
    ...
    private static
    ByteArrayOutputStream copyInputStream(InputStream inputStream) throws IOException{
        int longitud;
    ...
    public
    RequestBuilder setBody(Object set){
     ...
     */
    public
    RequestBuilder get(String uri) throws AmbiguousMethodException{
        setUp(uri, HttpMethod.GET);
        return this;
    }

    private
    void setUp(String uri, HttpMethod method) throws AmbiguousMethodException{
        if(this.method == null){
            this.uri = uri;
            this.method = method;
        }
        else{
            throw new AmbiguousMethodException(String.format("%s %s %s %s %s %s",
                                                             "Creating a request using 2 methods at the same time is " +
															 "not allowed",
     ...
     */
    public
    RequestBuilder post(String uri) throws AmbiguousMethodException{
     ...
     */
    public
    RequestBuilder put(String uri) throws AmbiguousMethodException{
     ...
     */
    public
    RequestBuilder delete(String uri) throws AmbiguousMethodException{
        setUp(uri, HttpMethod.DELETE);
        return this;
    }

    private
    RequestBuilder setHeaders(HttpURLConnection connection){
    ...
    Response perform() throws IOException{
        ...
        try{
            ...
            HttpUtils.setContentHeader(contentHeaders, body, headers.get("Content-type")).forEach(value -> {
    ...
    private
    void createBody(HttpURLConnection connection) throws JsonProcessingException{
        if(body != null){
